...
namespace SeguimientoCobrador.Collectable.PostgresImpl
{
    public class Account : CommonBase
    {
        ...
        public DataTable Assigned(int collectorId)
        {
            DataSet ds = new DataSet();
            NpgsqlDataAdapter da;
            string sqlString = "SELECT ctrl_cuenta.id_doco, f_documento, f_vencimiento, f_cobro, ctrl_cuenta.id_cliente, cd_cliente, nombre_cliente, ruta, dia_pago, " +
                ...
                "WHERE cat_cobrador.id_cobrador = " + collectorId.ToString() + " AND ctrl_cuenta.id_doco NOT IN(SELECT id_doco FROM ctrl_seguimiento WHERE id_movimiento IN(4,9,10));";
